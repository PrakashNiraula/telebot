...
db.createcategory=(name)=>{
    return new Promise((resolve,reject)=>{
        config.query("insert into category values (Null,?)",[name],(err,results)=>{
           if(err){
               return reject(err);
           } 
           return resolve(results);
    ...
    db.getcategoryByid=(id)=>{
        return new Promise((resolve,reject)=>{
            config.query("select * from category where cat_id=?",[id],(err,results)=>{
               if(err){
                   return reject(err);
               } 
               config.query("select * from subcategory where cat_id=?",[id],(error,results2)=>{
                if(error){
                    return reject(error);
                } 
        ...
        db.deletecategoryByid=(id)=>{
            return new Promise((resolve,reject)=>{
                config.query("delete from category where cat_id=?",[id],(err,results)=>{
                   if(err){
                       return reject(err);
                   } 
                   return resolve(results);
